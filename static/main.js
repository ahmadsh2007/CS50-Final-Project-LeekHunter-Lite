...
const displayIssues = (issues) => {
    ...
    issues.forEach(issue => {
        ...
        row.className = `border-b ${severityColors[severity] || severityColors['info']}`;
        ...
        row.innerHTML = `
            ...
            <td class="px-4 py-3">
                <span class="px-2 py-1 text-xs font-semibold rounded-full capitalize 
                    ...
                    ${severity === 'warning' ? 'bg-amber-200 text-amber-800' : ''}
            ...
            <td class="px-4 py-3">
                <pre class="code-suggestion text-xs bg-[#282c34] text-slate-300 p-2 rounded-md whitespace-pre-wrap break-words">${issue.suggestion || ''}</pre>
            </td>
        `;
...
analyzeButton.addEventListener('click', async () => {
    ...
    try {
        ...
        const errorIssue = [{
            line: 'N/A', severity: 'error',
            message: 'Failed to analyze code. There might be an issue with the backend server.',
